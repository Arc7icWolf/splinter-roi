...
def get_selling_prices(cards, gold, bcx, session: requests.Session):
    ...
    for card in cards_on_market:
        ...
        if card_id in card_ids and card["gold"] == gold:
            cards_list.append({"id": card_id, "price": card["low_price_bcx"] * bcx})

    return cards_list


def get_valid_active_rentals(active_rentals, past_days, gold, bcx):
...
def get_active_rentals(cards, gold, bcx, session: requests.Session):
    today = datetime.now()
    ...
    for card in cards:
        ...
        valid_active_rentals = get_valid_active_rentals(
            active_rentals, past_days, gold, bcx
        )
        card_rentals.append(
            {
                "name": card["name"],
...
def check_rental_roi(edition, types, rarity, gold, bcx, colours, session: requests.Session):
    cards = get_cards(edition, types, rarity, colours, session)

    card_selling_prices = get_selling_prices(cards, gold, bcx, session)

    card_rentals = get_active_rentals(cards, gold, bcx, session)
...
# def main(edition, types, rarity, gold, bxc, colours):
def main():
    edition = ["12"]  # Rebellion
    types = ["Summoner"]  # "Summoner" and/or "Monster"
    rarity = [1, 2]  # 1, 2, 3, and/or 4
    gold = True  # False or True
    bcx = 5
    colours = []

    try:
        with requests.Session() as session:
            result = check_rental_roi(edition, types, rarity, gold, bcx, colours, session)
